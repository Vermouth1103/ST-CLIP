...
from trainers.evaluator_build import build_evaluator
...
class SimpleTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        self.build_model()
        self.evaluator = build_evaluator(cfg, lab2cname_list=self.lab2cname_list)
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            input, label_list = self.parse_batch_test(batch)
            logits_list = self.model_inference(input)
            self.evaluator.process(logits_list, label_list)
